...
# Parsing and filtering of VCF files
					...
					for col in range(sampleformatcol+1, len(linearr)):
						...
						for tag in strfields:
							...
							if tag=="GT":
								...
								elif re.search(",", strarr[tagi]):
									thissam[tag] = strarr[tagi]
								else:
...
class SNP(object):
	...
	def __init__(self, vcf):
		pos = vcf.keys()[0]
		self.data = vcf.data[pos]
		self.chrom, self.pos = pos.split('@')
...
class SNPIterator(object):
        ...
        def __init__(self, fileName, filt=None):
	...
	def next(self):
			...
			for line in self.file:
				...
				v.sk = set (v.keys())
				if self.filter:
					v = v.filter(self.filter)
					if not v:
						continue
				assert len(v) == 1
				snp = SNP(v)
				if self._prevSNP and snp < self._prevSNP:
					raise SortingError(snp, "input not sorted")
				self._prevSNP = snp
				return SNP(v)
